...
import type { ArrayBufferBlob } from "matrix-files-sdk";
...
function getBlobSafeMimeType(mimetype: string): string {
...
export function createObjectUrl(blob: ArrayBufferBlob): string {
  return URL.createObjectURL(new Blob([blob.data], { type: getBlobSafeMimeType(blob.mimetype) }));
}
